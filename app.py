...
class VideoThread(QThread):
    ...
    def __init__(self, source=0, model_path="yolo_models\\Base.pt", tracking_path=None, use_gpu=False,ui_label=None,log_callback=None):
        super().__init__()
        self.source = source
...
class ConnectionScreen(QWidget):
    def __init__(self, switch_callback):
        ...
        self.model_path = "yolo_models/Base.pt"   # default model
    ...
    def init_ui(self):
        ...
        self.tracking_combo.addItems([
            "Default Tracking (OSNet Market1501)",
        ...
        self.tracking_path = "tracking_models/osnet_ain_x1_0_market1501.pth"
    ...
    def select_custom_model(self, index):
        ...
        if index == 1:  # Custom Model
            ...
            if file_path:
                ...
                self.model_path = "yolo_models\\Base.pt"
        else:
            self.model_path = "yolo_models\\Base.pt"
    ...
    def select_custom_tracking(self, index):
        ...
        DEFAULT_TRACKING_MODEL = (
            "tracking_models/osnet_ain_x1_0_market1501.pth"
        )
    ...
    def handle_mode(self, mode):
        ...
        if mode == "detection":
            ...
            if self.btn_full.styleSheet() == self.active_style:
                ...
                self.thread.set_mode("idle")
                self.is_detection_running = False
                # Disable Full Monitor mode in backend
                self.thread.backendUI.backend.set_full_monitor_mode(False)
                ...
                self.thread.set_mode("tracking")
                self.is_detection_running = True
                # Save Violations automatically on Full Monitor
                self.thread.backendUI.backend.set_full_monitor_mode(True)



                # Create default save directory
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                save_root = f"Full_Monitor_Recordings/Full_Monitor_Violations_{timestamp}"
...
class ViolationScreen(BaseMonitorScreen):
    ...
    def add_left_sidebar_buttons(self, layout):
        """Add update button and confidence control for violation classes"""
        button_container = QWidget()
        button_layout = QVBoxLayout(button_container)
        button_layout.setContentsMargins(15, 10, 15, 20)
        button_layout.setSpacing(15)

        # ✅ FIX 1: Update Violation Classes Button
        ...
        # ✅ FIX 2: Add separator line
        separator = QFrame()
        separator.setFrameShape(QFrame.HLine)
        separator.setFrameShadow(QFrame.Sunken)
        separator.setStyleSheet(f"background-color: #374151; margin: 5px 0;")
        button_layout.addWidget(separator)

        # ✅ FIX 3: Confidence Control Section
        conf_title = QLabel("Detection Settings")
        conf_title.setStyleSheet(
            f"color: {PRIMARY_COLOR}; font-weight: bold; font-size: 13px; "
            "padding-top: 5px;"
        )
        button_layout.addWidget(conf_title)

        # ✅ FIX 4: Confidence spinbox with proper layout
        conf_layout = QHBoxLayout()
        conf_layout.setSpacing(8)

        conf_label = QLabel("Min Confidence:")
        conf_label.setStyleSheet(f"color: {TEXT_COLOR}; font-size: 12px;")
        conf_layout.addWidget(conf_label)

        self.conf_spinbox = QSpinBox()
        self.conf_spinbox.setRange(50, 95)
        self.conf_spinbox.setValue(80)
        self.conf_spinbox.setSuffix("%")
        self.conf_spinbox.setFixedHeight(32)
        self.conf_spinbox.setCursor(Qt.PointingHandCursor)
        self.conf_spinbox.setStyleSheet(f"""
            QSpinBox {{
                background-color: {BACKGROUND_COLOR};
                border: 1px solid #374151;
                border-radius: 4px;
        ...
        button_layout.addLayout(conf_layout)

        # ✅ FIX 5: Add info label
        conf_info = QLabel("⚠️ Only persons above this\nconfidence will be checked")
        conf_info.setWordWrap(True)
        conf_info.setStyleSheet(
            f"color: {SECONDARY_TEXT}; font-size: 11px; "
            f"background-color: rgba(245, 158, 11, 0.1); "
            "padding: 8px; border-radius: 4px; border-left: 2px solid #F59E0B;"
        )
        button_layout.addWidget(conf_info)

        # ✅ FIX 6: Add stretch to push everything to top
        button_layout.addStretch()

        # ✅ FIX 7: Add container to parent layout
        layout.addWidget(button_container)

    def update_confidence(self, value):
        """Update minimum person confidence threshold"""
        conf = value / 100.0

        if hasattr(self, 'violation_backend') and self.violation_backend:
            if hasattr(self.violation_backend, 'ViolationDetector'):
                self.violation_backend.ViolationDetector.set_min_person_confidence(conf)
                self.append_log(f"🔧 Confidence threshold updated: {conf:.2f} ({value}%)", log_type="INFO")
            else:
                self.append_log("⚠️ ViolationDetector not found", log_type="WARNING")
        else:
            self.append_log("⚠️ Backend not initialized", log_type="WARNING")
